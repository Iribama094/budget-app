 ...
 * Without: join someone else's budget with a code.
 */
export default function ShareBudgetScreen() {
  ...
  const budgetName: string = String(route.params?.budgetName ?? 'this budget').replace(/^My Budget \((.*)\)$/, '$1');
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await listBudgetMembers(budgetId);
      setRole(res.role);
  ...
  const shareInvite = async () => {
    if (!invite) return;
    await Share.share({
      message: `Join my BudgetFriendly budget "${budgetName}". Open BudgetFriendly, go to Budgets › Join a shared budget, and enter code ${invite.code}. It expires on ${formatShortDate(invite.expiresAt)}.`
    });
  };

  const remove = (m: ApiBudgetMember) => {
    const leaving = m.userId === user?.id;
    Alert.alert(leaving ? `Leave ${budgetName}?` : `Remove ${m.name || m.email}?`, leaving ? 'You’ll stop seeing this budget. Your transactions stay in your own history.' : 'They’ll lose access to this budget. Transactions they added stay.', [
  ...
  const join = async () => {
    ...
    try {
      ...
      toast.show(`You joined ${budget.name.replace(/^My Budget \((.*)\)$/, '$1')}`, 'success');
      nav.replace('BudgetDetail', { budgetId: budget.id });
    } catch (e) {
  ...
  const joinSection = (
    <>
      ...
      <Card>
        <Text style={[type.small, { color: theme.colors.textMuted, marginBottom: 12 }]}>Got a code from a partner or housemate? Enter it to budget together.</Text>
        <TextField
          label="Invite code"
          value={code}
        ...
        <Card style={{ marginTop: 14 }}>
          <View style={{ flexDirection: 'row', gap: 12, alignItems: 'center' }}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={[type.caption, { color: theme.colors.textMuted }]}>Budget together with a partner or housemate</Text>
            </View>
          </View>
          <Text style={[type.small, { color: theme.colors.textMuted, marginTop: 12 }]}>
            Everyone’s spending in this budget counts toward it, and everyone gets pace alerts. Your other budgets, goals and transactions stay private.
          </Text>
        </Card>
      ) : null}
              ...
              {invite ? (
                ...
                <Card>
                  <Text style={[type.small, { color: theme.colors.textMuted, marginBottom: 12 }]}>
                    Create a one-time code and send it by WhatsApp or SMS. They enter it in BudgetFriendly to join.
                  </Text>
