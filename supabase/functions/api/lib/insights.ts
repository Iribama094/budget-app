...
export type Insight = {
  key: string;
  kind: 'runway' | 'spike' | 'small_spends' | 'regular_bill' | 'kept' | 'overspent' | 'plan_drift' | 'logging_gap' | 'irregular_income';
...
export async function computeInsights(userId: string, space: Space = 'personal', today = todayIso(), options: { includeDismissed?: boolean } = {}): Promise<Insight[]> {
    ...
    if (elapsed >= 3 && spent > 0 && remaining > 0) {
      ...
      if (lasts < remaining - 1) {
        ...
        out.push({
          ...
          kind: 'runway',
          tone: 'warning',
          title: left > 0 ? `At this pace, money runs out ${plural(short, 'day')} before ${window.until}` : `You’ve spent more than planned for this period`,
          body:
            left > 0
        ...
        out.push({
          ...
          kind: 'runway',
          tone: 'positive',
          title: `You’re on track for ${window.until}`,
          body: `About ${money(left / (remaining + 1))} a day keeps you on plan for the next ${plural(remaining, 'day')}.`,
          action: null
        });
      }
    }
  ...
  if (oldest >= 45) {
    ...
    if (spikes[0]) {
      ...
      out.push({
        ...
        kind: 'spike',
        tone: 'warning',
        title: `${s.category} is up ${Math.round((s.now / s.usual - 1) * 100)}%`,
  ...
  if (regular[0]) {
    ...
    out.push({
      ...
      kind: 'regular_bill',
      tone: 'neutral',
      title: `Looks like a regular bill: ${r.label.slice(0, 40)}`,
  ...
  if (in30 > 0) {
    ...
    if (rate >= 0.1) {
      out.push({
        ...
        kind: 'kept',
        tone: 'positive',
        title: `You kept ${Math.round(rate * 100)}% of what came in`,
    ...
    if (recent.length >= 15) {
      ...
      if (share > plan.percents.Needs + 10) {
        out.push({
          ...
          kind: 'plan_drift',
          tone: 'neutral',
          title: 'Needs cost more than your plan',
    ...
    if (values.length >= 2) {
      ...
      if (high > low * 1.3) {
        out.push({
          ...
          tone: 'neutral',
          title: 'Your income changes month to month',
