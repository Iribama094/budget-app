...
How to help:
- Speak plainly and kindly, like a patient friend who is good with money. No shame, no lectures.
...
/** Answers a question with Claude, grounded in a summary of the person's own data. */
export async function assistantReply(userId: string, message: string, history: ChatTurn[]): Promise<string> {
  const apiKey = Deno.env.get('ANTHROPIC_API_KEY');
  if (!apiKey) throw new HttpError(501, 'NOT_CONFIGURED', 'Flux isn’t switched on yet. Ask the app owner to add the AI key.');
  ...
  try {
    res = await fetch(ANTHROPIC_URL, {
      ...
      body: JSON.stringify({
        model: Deno.env.get('ASSISTANT_MODEL') || DEFAULT_MODEL,
        max_tokens: 700,
  ...
  if (!res.ok) {
    console.error('[assistant] provider error', res.status, (await res.text().catch(() => '')).slice(0, 500));
    if (res.status === 429 || res.status === 529 || res.status === 503) throw new HttpError(503, 'ASSISTANT_BUSY', 'Flux is busy right now. Try again in a minute.');
    if (res.status === 401 || res.status === 403) throw new HttpError(501, 'NOT_CONFIGURED', 'Flux isn’t set up correctly yet. Ask the app owner to check the AI key.');
    throw new HttpError(502, 'ASSISTANT_ERROR', 'Flux couldn’t answer just now. Try again.');
  }
  ...
  const reply = (out?.content ?? [])
    ...
    .join('\n')
    .trim();
  return reply || 'Sorry, I don’t have an answer for that yet.';
}
